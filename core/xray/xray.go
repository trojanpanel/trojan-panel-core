...
func XrayRemoveHandler() {
	...
	common.Must(err)
	if resp == nil {
		logrus.Error("unexpected nil response")
	}

	{
		...
		if err == nil {
			logrus.Error("unexpected nil error")
		}
	}
}

func XrayAddRemoveUser() {
	tcpServer := tcp.Server{
		MsgProcessor: xor,
	}
	dest, err := tcpServer.Start()
	...
	if err := testTCPConn(clientPort, 1024, time.Second*5)(); err != io.EOF &&
		...
		logrus.Fatal("expected error: ", err)
	}
...
func XrayStats() {
	...
	servers, err := InitializeServerConfigs(serverConfig, clientConfig)
	if err != nil {
		logrus.Fatal("Failed to create all servers", err)
	}
	defer CloseAllServers(servers)
	...
	if sresp.Stat.Value <= 10240*1024 {
		logrus.Error("value < 10240*1024: ", sresp.Stat.Value)
	}
}
